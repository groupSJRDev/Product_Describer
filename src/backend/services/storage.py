...
class StorageService:
    ...
    def _get_generated_path(self, product_slug: str) -> Path:
        ...
        now = datetime.now()
        path = (
            self._get_product_path(product_slug)
            / "generated"
            / str(now.year)
            / f"{now.month:02d}"
        )
    ...
    def _generate_filename(self, original_filename: str, prefix: str = "") -> str:
        ...
        hash_part = hashlib.md5(f"{original_filename}{timestamp}".encode()).hexdigest()[
            :8
        ]
    ...
    def save_reference_image(
        self, product_slug: str, file: BinaryIO, original_filename: str
    ) -> tuple[str, str]:
        """
    ...
    def save_specification(
        self, product_slug: str, yaml_content: str, version: int
    ) -> str:
        """
